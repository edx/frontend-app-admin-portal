...
const ApprovedRequestBulkRemindAction = ({
  ...
  const {
    BUDGET_DETAILS_PENDING_DATATABLE_OPEN_BULK_REMIND_MODAL,
    BUDGET_DETAILS_PENDING_DATATABLE_CLOSE_BULK_REMIND_MODAL,
    BUDGET_DETAILS_PENDING_DATATABLE_BULK_REMIND,
  } = EVENT_NAMES.LEARNER_CREDIT_MANAGEMENT;

  const trackEvent = (eventName) => {
    // Constructs a learner request state object for the select all state
    const learnerRequestStateObject = {};
    learnerRequestStateCounts.forEach((item) => {
      learnerRequestStateObject[item.learnerRequestState] = item.count;
    });
  ...
  const openModal = () => {
    open();
    trackEvent(BUDGET_DETAILS_PENDING_DATATABLE_OPEN_BULK_REMIND_MODAL);
  };

  const closeModal = () => {
    close();
    trackEvent(BUDGET_DETAILS_PENDING_DATATABLE_CLOSE_BULK_REMIND_MODAL);
  };

  const reminderTrackEvent = () => {
    trackEvent(BUDGET_DETAILS_PENDING_DATATABLE_BULK_REMIND);
  };
